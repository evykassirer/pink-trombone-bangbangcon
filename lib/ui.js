...
var UI =
{
    ...
    handleKeydown : function(event) {
        ...
        // todo: move IPA logic elsewhere
        else if (event.keyCode === 77 ) { // m
            UI.addSimulatedTouch({
                x: 483.45784418356453,
                y: 101.81430096051227,
            })
            UI.setVoice(true);
        }
        else if (event.keyCode === 65) { // ɑ (pat)
          UI.addSimulatedTouch({
    ...
    removeSimulatedTouches(noMore=false) {
    ...
    updateTouches : function()
    {
        ...
        for (var j=UI.touchesWithMouse.length-1; j >=0; j--)
        {
            ...
            else if (touch.alive || touch.keepingWithExploreMode)
            {
